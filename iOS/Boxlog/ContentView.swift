    ...
    private func row(_ entry: SessionEntry) -> some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Rounds: \(entry.value1)  \u{00B7}  Round Length (min): \(entry.value2)")
                    .font(.caption)
...
struct EntrySheet: View {
    ...
    @State private var option: String = BoxlogOptions.all.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Combo") {
                    Picker("Combo", selection: $option) {
                        ForEach(BoxlogOptions.all, id: \.self) { t in
                            Text(t).tag(t)
                        }
                    }
                    .pickerStyle(.menu)
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        if var existing = entry {
                            existing.combo = option
                            existing.value1 = Int(value1)
                            existing.value2 = Int(value2)
                            existing.note = note
                            store.update(existing)
                        } else {
                            store.add(combo: option, value1: Int(value1), value2: Int(value2), note: note)
                        }
                        dismiss()
                    }
